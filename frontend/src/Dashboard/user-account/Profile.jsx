...
const Profile = ({user}) => {
    ...
    useEffect(() => {
        setFormData({name:user.name, email:user.email, address:user.address, photo:user.photo, gender:user.gender, phone:user.phone })
    }, [user])
    ...
    <div className="mt-10">
        <form onSubmit={submitHandler}>
                    ...
                    type="submit" 
                    className="w-full bg-primaryColor text-white text-[18px] leading-[30px] rounded-lg px-4 py-3">{loading ? <HashLoader size={25} color="#FFFF"/> : 'Update'}</button>
                   </div>
        </form>
    </div>
    );
